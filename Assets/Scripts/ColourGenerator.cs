...
public class ColourGenerator
{
    ...
    public float BiomePercentFromPoint(Vector3 pointOnUnitSphere)
    {
        ...
        float blendRange = settings.biomeColourSettings.blendAmout / 2f + 0.001f;

        for (int i = 0; i < numBiomes; i++)
        {
            float distacne = heightPercent - settings.biomeColourSettings.biomes[i].startHeight;
            float weight = Mathf.InverseLerp(-blendRange, blendRange, distacne);
            biomeIndex *= (1 - weight);
            biomeIndex += i * weight;
        }
